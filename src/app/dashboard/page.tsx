...
import { DollarSign, Home, AlertCircle, Wrench, BarChart3, PieChart } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const occupiedUnits = properties.reduce(
      (sum, prop) => sum + (prop.units?.filter((u: any) => u.status === 'OCCUPIED').length || 0),
      0
    );
    const vacantUnits = totalUnits - occupiedUnits;

    const paidPayments = payments.filter((p: any) => p.status === 'PAID');
    const monthlyRevenue = paidPayments.reduce((sum, p: any) => sum + Number(p.amount), 0);
    const outstandingPayments = payments.filter((p: any) => p.status === 'PENDING' || p.status === 'OVERDUE');
    const outstandingAmount = outstandingPayments.reduce((sum, p: any) => sum + Number(p.amount), 0);

    const pendingMaintenance = requests.filter((r: any) => r.status === 'PENDING' || r.status === 'IN_PROGRESS').length;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="h-64 flex items-center justify-center">
            {payments.length > 0 ? (
              <div className="text-center w-full">
                ...
                <p className="text-slate-600 mb-6">Total Revenue</p>
                <div className="grid grid-cols-2 gap-4 max-w-md mx-auto">
                  <div className="p-4 bg-emerald-50 rounded-lg">
                    <div className="text-2xl font-bold text-emerald-700">{stats.paidPayments.length}</div>
                    <div className="text-sm text-slate-600">Paid</div>
                  </div>
                  <div className="p-4 bg-amber-50 rounded-lg">
                    <div className="text-2xl font-bold text-amber-700">{stats.outstandingPayments.length}</div>
        ...
        {/* Occupancy Chart */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          <h3 className="text-lg font-semibold text-slate-900 mb-4">Unit Occupancy</h3>
          <div className="h-64 flex items-center justify-center">
            {stats.totalUnits > 0 ? (
              <div className="text-center">
                ...
                <div className="mt-4 space-y-2">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <span className="text-sm text-slate-600">Occupied: {stats.occupiedUnits}</span>
                  </div>
                  <div className="flex items-center justify-center gap-2">
                    <div className="w-3 h-3 bg-slate-300 rounded-full"></div>
                    <span className="text-sm text-slate-600">Vacant: {stats.vacantUnits}</span>
                  </div>
                </div>
              </div>
            ) : (
      ...
      {properties.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-12">
          <div className="text-center max-w-md mx-auto">
            ...
            <p className="text-slate-600 mb-6">
              Add your first property to start managing tenants, tracking payments, and more.
            </p>
            <button
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200">
          <div className="p-6 border-b border-slate-200 flex items-center justify-between">
            <h3 className="text-lg font-semibold text-slate-900">Your Properties</h3>
            <button
              onClick={() => handleNavigate('/dashboard/properties')}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {properties.slice(0, 3).map((property) => (
                <div
                  ...
                  onClick={() => handleNavigate(`/dashboard/properties`)}
                >
                  <h4 className="font-semibold text-slate-900 mb-1">{property.name}</h4>
                  <p className="text-sm text-slate-600 mb-2">{property.address}</p>
                  <div className="flex items-center gap-4 text-xs text-slate-500">
                    <span>{property.units?.length || 0} units</span>
                    <span>•</span>
                    <span>
                      {property.units?.filter((u: any) => u.status === 'OCCUPIED').length || 0} occupied
                    </span>
                  </div>
                </div>
              ))}
              ...
              {recentPayments.map((payment: any) => (
                <div key={payment.id} className="flex items-center justify-between p-3 border border-slate-200 rounded-lg">
                  <div className="flex-1">
                    <div className="font-medium text-slate-900">{payment.tenant.user.fullName}</div>
                    <div className="text-sm text-slate-500">
                      {payment.lease.unit.property.name} - Unit {payment.lease.unit.unitNumber}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="font-semibold text-slate-900">UGX {Number(payment.amount).toLocaleString()}</div>
                    <div className={`text-xs ${payment.status === 'PAID' ? 'text-emerald-600' : 'text-amber-600'}`}>
                      {payment.status}
                    </div>
                  </div>
                </div>
