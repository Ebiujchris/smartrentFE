...
import { paymentService, CreatePaymentDto, UpdatePaymentDto } from '@/services/payment.service';
...
interface PaymentStore {
  ...
  recordPayment: (id: string, method: string, reference?: string) => Promise<any>;
...
export const usePaymentStore = create<PaymentStore>((set) => ({
  ...
  recordPayment: async (id: string, method: string, reference?: string) => {
    set({ loading: true, error: null });
    try {
      const updated = await paymentService.recordPayment(id, method, reference);
      set((state) => ({
        payments: state.payments.map((p) => (p.id === id ? updated : p)),
        loading: false,
      }));
